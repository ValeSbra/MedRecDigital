...
## GSV data, updated SEPTEMBER 2025
gsv_data <- read_rds("GSV_data_2013_2024.rds") %>% 
...
This public observatory integrates emerging digital approaches to provide a cohesive perspective on how Google Search Volume (i.e., a proxy for public attention) related to Mediterranean marine fish species has changed across time and space. The species included here are those that have at least one measurement available for a country.
...
renderUI({
  ...
  format_trait <- function(label, value, is_numeric = FALSE) {
    if (is.na(value) || value == "NA" || value == "" || is.null(value)) {
      value_clean <- "No data available"
    } else {
      if (is_numeric) {
        # Convertir a numérico y formatear con 1 decimal, manejando posibles errores
        tryCatch({
          numeric_value <- as.numeric(as.character(value))
          if (!is.na(numeric_value)) {
            value_clean <- sprintf("%.1f", numeric_value)
          } else {
            value_clean <- "No data available"
          }
        }, error = function(e) {
        ...
        # Para valores no numéricos, aplicar title case
        value_clean <- tools::toTitleCase(tolower(as.character(value)))
      }
    }
  ...
  trait_lines <- glue::glue_collapse(c(
    format_trait("Length (cm):", data$length[1], is_numeric = TRUE),
    format_trait("Weight (g):", data$weight[1], is_numeric = TRUE),
    format_trait("Habitat:", data$habitat[1], is_numeric = FALSE),
    format_trait("Feeding:", data$feeding[1], is_numeric = FALSE)
  ), sep = "<br>")
  ...
  map_data <- world %>%
    ...
    mutate(
      ...
      tooltip_text = case_when(
        !is.na(sum_gsv) & sum_gsv >= 0 ~ paste0("<b>", region, ":</b> ", sprintf("%.2f", sum_gsv)),
        region %in% study_countries ~ paste0("<b>", region, ":</b> ", "0.00"),
        TRUE ~ region
      )
    )
  
  p <- ggplot(map_data, aes(x = long, y = lat, group = group, fill = fill_color, text = tooltip_text)) +
    ...
    coord_map(xlim = c(-10, 40), ylim = c(30, 50)) +
    theme(
    ...
    guides(
      fill = guide_colorbar(
        ...
        title.hjust     = 0.5,
        barwidth        = unit(60, "points"),
        barheight       = unit(30, "points")
      )
    )
  
   ggplotly(p, tooltip = "text", height = 412) %>%
     config(
    ...
    function(el, x) {
      ...
      el.removeAllListeners('plotly_selecting');
...
div(
  ...
  withSpinner(
    plotlyOutput("gsv_map", height = "412px"),
    type = 6,           # estilo del spinner
    color = "#0077b6",  # color del spinner
    proxy.height = "412px"
  )
)
# Mensaje fijo informativo debajo del gráfico
div(
  id = "gsv_map_message",
  style = "color: black; font-size: 14px; margin-top: 5px; text-align: right; padding-right: 10px;",
  "Percentage of sum of annual Google Search Volume (GSV) values are displayed. Percentage = 0 is in gray."
)
# CSS que añade el mensaje mientras carga
tags$style(HTML("
  .plot-container .load-container:after {
    content: '⏳ Please wait, your results will be ready in about 10 seconds...';
    display: block;
    text-align: center;
    color: black;
    margin-top: 10px;
...
```{r}
output$gsv_year_plot <- renderPlotly({
  data_plot <- filtered_species() %>% 
    group_by(year) %>%
    ...
    mutate(
      ...
      tooltip_text = paste0("<b>", year, ":</b> ", sprintf("%.2f", scaled_gsv))
    ) %>%
    filter(!is.na(scaled_gsv))
    
    p <- ggplot(data_plot, aes(x = year, y = scaled_gsv, text = tooltip_text)) + 
    geom_bar(stat = "identity", fill = "#00b4d8") +
    labs(x = "Year", y = "Percentage of Google Search Volume") +
    theme_base()
    ...
    ggplotly(p, tooltip = "text") %>%
     ...
     layout(
       dragmode = FALSE,
       xaxis = list(
         fixedrange = TRUE,  # Evita zoom en X
         showspikes = FALSE
       ),
       yaxis = list(
         fixedrange = TRUE,  # Evita zoom en Y
         showspikes = FALSE
       ),
       hoverlabel = list(
          bgcolor = "white",
...
div(
  withSpinner(
    plotlyOutput("gsv_year_plot", height = "368px"),
    type = 6,
    color = "#0077b6",
    proxy.height = "368px"
  )
...
output$gsv_country_plot <- renderPlotly({
  ...
  ggplotly(p) %>% config(displayModeBar = FALSE)
    ...
    data_plot <- data %>%
     ...
     ggplotly(p, tooltip = "text", height = 368) %>%
     config(
      displayModeBar = FALSE,
      scrollZoom = FALSE,
      doubleClick = FALSE,
...
div(
  withSpinner(
    plotlyOutput("gsv_country_plot", height = "368px"),
    type = 6,
    color = "#0077b6",
    proxy.height = "368px"
  )
